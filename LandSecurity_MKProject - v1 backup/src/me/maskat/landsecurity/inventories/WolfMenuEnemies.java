...
public class WolfMenuEnemies implements InventoryHolder
{
    ...
    private void initializeItems()
    {
        ...
        inv.setItem(InvSlot.back(),WolfMenu.createGuiItemParseLang(Material.ENDER_PEARL, Message.getColorMessageLang("inventory.wolfmenu.wolf.manageenemies.submenu.back.slot_title")));
        inv.setItem(InvSlot.showUsers(),WolfMenu.createGuiItemParseLang(Material.PLAYER_HEAD, Message.getColorMessageLang("inventory.wolfmenu.wolf.manageenemies.submenu.showusers.slot_title")));
        inv.setItem(InvSlot.addUsers(),WolfMenu.createGuiItemParseLang(Material.NETHER_STAR, Message.getColorMessageLang("inventory.wolfmenu.wolf.manageenemies.submenu.addusers.slot_title")));
    }
	...
	public void onInventoryClick(final InventoryClickEvent e, final Player player)
	{
        ...
        if(clickedSlot == InvSlot.showUsers()) {
        	Model.Player(player).getWolfMenu().openEnemiesShowUsersInventory();
        	return;
        }
        if(clickedSlot == InvSlot.addUsers()) {
        	Model.Player(player).getWolfMenu().openEnemiesAddUsersInventory();
        	return;
        }
	}
