...
public class MenuMain implements InventoryHolder {
	...
	private void initializeItems(final Player player)
    {
        ...
        inv.setItem(0,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(8,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(9,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(17,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(18,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(26,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(27,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(35,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(36,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(44,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(45,InventoryMenu.createGuiItem(Material.COMPASS));
        inv.setItem(53,InventoryMenu.createGuiItem(Material.COMPASS));
        
        inv.setItem(InvSlot.navigateHelp(),InventoryMenu.createGuiItemParseMsg(Material.BOOK, Message.getColorMessage("&l*** POMOC ***\nTrzymając w ręku kompas możesz kliknąć\nlewym przyciskiem myszy w oddalony punkt,\naby utworzyć chwilowy \"wskaźnik\", który\nmogą zobaczyć tylko członkowie twojego party.")));
        
        inv.setItem(InvSlot.navigateInfo(),InventoryMenu.createGuiItemParseMsg(Material.REDSTONE_TORCH, Message.getColorMessage("&2Aktualne ustawienie kompasu:\n"+Model.Player(player).getNavLocationInfo())));
        
        if(Model.Player(player).existBedSpawn())
        	inv.setItem(InvSlot.navigateToBed(),InventoryMenu.createGuiItemParseMsg(Material.RED_BED, Message.getColorMessage("&6Nawiguj do &ełóżka")));
        else
        	inv.setItem(InvSlot.navigateToBed(),InventoryMenu.createGuiItemParseMsg(Material.RED_BED, Message.getColorMessage("&6Nawiguj do &ełóżka\n&c&oNiedostępne - brak łóżka")));
        
        if(Model.Player(player).existWolfAPI())
        	inv.setItem(InvSlot.navigateToWolf(),InventoryMenu.createGuiItemParseMsg(Material.BONE, Message.getColorMessage("&6Nawiguj do &ewilka")));
        else
...
//    
		...
		if(partyPlayers.size() > 1)
		{
			...
			for(PartyPlayer partyPlayer : partyPlayers)
			{
				...
				this.setTargetPlayerSlot(InvSlot.navigateToPlayerSlots()[i], targetPlayer);
				if(targetPlayer.getWorld().getName().equals("world"))
					inv.setItem(InvSlot.navigateToPlayerSlots()[i],InventoryMenu.createGuiItemParseMsg(Material.PLAYER_HEAD, Message.getColorMessage("&6Nawiguj do &e" + targetPlayer.getName())));
				else
		...
		Bukkit.getScheduler().runTaskAsynchronously(Plugin.getPlugin(), new Runnable() {
			...
			public void run() {
				for (Map.Entry<Integer, Player> entry : targetPlayerSlotList.entrySet()) {
					...
					if(targetPlayer.getWorld().getName().equals("world"))
						inv.setItem(entry.getKey(),InventoryMenu.createGuiItemHeadParseMsg(targetPlayer, Message.getColorMessage("&6Nawiguj do &e" + targetPlayer.getName())));
					else
						inv.setItem(entry.getKey(),InventoryMenu.createGuiItemHeadParseMsg(targetPlayer, Message.getColorMessage("&6Nawiguj do &e" + targetPlayer.getName() + "\n&7&oGracz jest aktualnie poza światem Survival")));
				}
			}
