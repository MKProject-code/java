package me.eccentric_nz.com.zaxxer.hikari.util;
...
public class ConcurrentBag<T extends ConcurrentBag.IConcurrentBagEntry> implements AutoCloseable {
   ...
   public ConcurrentBag(ConcurrentBag.IBagStateListener listener) {
      ...
      if (this.weakThreadLocals) {
         ...
         this.threadList = ThreadLocal.withInitial(() -> {
            return new FastList(ConcurrentBag.IConcurrentBagEntry.class, 16);
         });
      }
   ...
   public T borrow(long timeout, TimeUnit timeUnit) throws InterruptedException {
      ...
      int waiting;
      Object entry;
      ConcurrentBag.IConcurrentBagEntry bagEntry;
      for(waiting = list.size() - 1; waiting >= 0; --waiting) {
         entry = list.remove(waiting);
         bagEntry = this.weakThreadLocals ? (ConcurrentBag.IConcurrentBagEntry)((WeakReference)entry).get() : (ConcurrentBag.IConcurrentBagEntry)entry;
      ...
      try {
         label165: {
            ...
            while(var13.hasNext()) {
               bagEntry = (ConcurrentBag.IConcurrentBagEntry)var13.next();
            ...
            do {
               ...
               bagEntry = (ConcurrentBag.IConcurrentBagEntry)this.handoffQueue.poll(timeout, TimeUnit.NANOSECONDS);
               if (bagEntry == null || bagEntry.compareAndSet(0, 1)) {
                  ConcurrentBag.IConcurrentBagEntry var9 = bagEntry;
                  return var9;
               }
            ...
            entry = null;
            return (ConcurrentBag.IConcurrentBagEntry)entry;
         }
   ...
   public int getCount(int state) {
      ...
      while(var3.hasNext()) {
         ConcurrentBag.IConcurrentBagEntry e = (ConcurrentBag.IConcurrentBagEntry)var3.next();
         if (e.getState() == state) {
            ++count;
         }
      }
   ...
   public int[] getStateCounts() {
      ...
      ConcurrentBag.IConcurrentBagEntry e;
      for(Iterator var2 = this.sharedList.iterator(); var2.hasNext(); ++states[e.getState()]) {
         e = (ConcurrentBag.IConcurrentBagEntry)var2.next();
      }
   ...
   private boolean useWeakThreadLocals() {
      try {
         if (System.getProperty("me.eccentric_nz.com.zaxxer.hikari.useWeakReferences") != null) {
            return Boolean.getBoolean("me.eccentric_nz.com.zaxxer.hikari.useWeakReferences");
         } else {
