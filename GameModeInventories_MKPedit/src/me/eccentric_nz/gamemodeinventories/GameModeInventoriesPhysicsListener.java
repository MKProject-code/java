...
public class GameModeInventoriesPhysicsListener implements Listener {
   ...
   public void onBlockPhysics(BlockPhysicsEvent event) {
      if (this.willdrop.contains(event.getBlock().getType())) {
         if (this.plugin.getConfig().getBoolean("track_creative_place.enabled") && this.plugin.getConfig().getBoolean("track_creative_place.attached_block")) {
            Block block = event.getBlock();
            if (this.plugin.getConfig().getStringList("track_creative_place.worlds").contains(block.getWorld().getName())) {
               if (!this.doors.contains(block.getType()) || !this.plates.contains(event.getChangedType())) {
                  if (this.willdrop.contains(block.getType())) {
                     ...
                     if (((List)this.plugin.getCreativeBlocks().get(gmiwc)).contains(event.getBlock().getLocation().toString())) {
