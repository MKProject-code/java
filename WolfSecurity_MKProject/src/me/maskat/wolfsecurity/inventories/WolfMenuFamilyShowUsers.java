...
public class WolfMenuFamilyShowUsers implements InventoryHolder
{
    ...
    {
    	int i=0;
    	for (int userid: Model.Region(Model.Player(player).getOwnRegionId()).getFamilyUsersIdList()) {
			InvSlot.putUserId(i, userid);
			inv.setItem(i,WolfMenu.createGuiItemHeadParseLang(userid, Message.getColorMessageLang("inventory.wolfmenu.wolf.managefamily.submenu.showusers.submenu.head.slot_title", ImmutableMap.of(
					"player_name", Model.User(userid).getName()
					))));
    		i++;
    	}
    	inv.setItem(InvSlot.back(),WolfMenu.createGuiItemParseLang(Material.ENDER_PEARL, Message.getColorMessageLang("inventory.wolfmenu.wolf.managefamily.submenu.showusers.submenu.back.slot_title")));
		...
		Bukkit.getScheduler().runTaskAsynchronously(Plugin.plugin, new Runnable() {
			...
			public void run() {
				for (Map.Entry<Integer, Integer> entry : InvSlot.users.entrySet()) {
	    			inv.setItem(entry.getKey(),WolfMenu.createGuiItemHeadParseLang(entry.getValue(), Message.getColorMessageLang("inventory.wolfmenu.wolf.managefamily.submenu.showusers.submenu.head.slot_title", ImmutableMap.of(
    ...
    public void onInventoryClick(final InventoryClickEvent e, final Player player)
	{
        ...
        if(InvSlot.existUser(clickedSlot))
        {
        	...
        	Model.Player(player).getOwnRegion().removeFamily(clickedUserid);
        	InvSlot.removeUser(clickedSlot);
        	Model.Player(player).getWolfMenu().initializeNewUsersInventoryWithout(this);
        	inv.setItem(clickedSlot, WolfMenu.createGuiItemHeadParseLang(clickedUserid, Message.getColorMessageLang("inventory.wolfmenu.wolf.managefamily.submenu.showusers.submenu.head.done.slot_title", ImmutableMap.of(
