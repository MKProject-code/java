...
public class WolfMenuFamilyAddUsers implements InventoryHolder
{
    ...
    private void initializeItems(Player player)
    {
    	...
    	for (Player serverplayer: Bukkit.getServer().getOnlinePlayers()) {
    		if(Model.Players.isExist(serverplayer) && serverplayer != player && !Model.Player(player).getOwnRegion().isFamily(serverplayer))
    		{
    			InvSlot.putUserId(i, Model.Player(serverplayer).getUserId());
    			inv.setItem(i,WolfMenu.createGuiItemHeadParseLang(Model.Player(serverplayer).getUserId(), Message.getColorMessageLang("inventory.wolfmenu.wolf.managefamily.submenu.addusers.submenu.head.slot_title", ImmutableMap.of(
    					"player_name", serverplayer.getName()
    					))));
    			i++;
    		}
    	}
    	inv.setItem(InvSlot.back(),WolfMenu.createGuiItemParseLang(Material.ENDER_PEARL, Message.getColorMessageLang("inventory.wolfmenu.wolf.managefamily.submenu.addusers.submenu.back.slot_title")));
		...
		Bukkit.getScheduler().runTaskAsynchronously(Plugin.plugin, new Runnable() {
			...
			public void run() {
				for (Map.Entry<Integer, Integer> entry : InvSlot.users.entrySet()) {
	    			inv.setItem(entry.getKey(),WolfMenu.createGuiItemHeadParseLang(entry.getValue(), Message.getColorMessageLang("inventory.wolfmenu.wolf.managefamily.submenu.addusers.submenu.head.slot_title", ImmutableMap.of(
    ...
    public void onInventoryClick(final InventoryClickEvent e, final Player player)
	{
        ...
        if(InvSlot.existUser(clickedSlot))
        {
        	...
        	Model.Player(player).getOwnRegion().addFamily(clickedUserid);
        	InvSlot.removeUser(clickedSlot);
        	Model.Player(player).getWolfMenu().initializeNewUsersInventoryWithout(this);
        	inv.setItem(clickedSlot, WolfMenu.createGuiItemHeadParseLang(clickedUserid, Message.getColorMessageLang("inventory.wolfmenu.wolf.managefamily.submenu.addusers.submenu.head.done.slot_title", ImmutableMap.of(
