package me.maskat.wolfsecurity.menu;
...
public class WolfMenu implements PapiMenu {
...
//				);
		...
		if(WolfSecurityApi.existWolf(player))
			this.papiMenuPage.setItem(InventorySlot.ROW3_COLUMN5, Material.BONE, "&aPosiadasz swojego wilka!\n&7Możesz zarzącać wilkiem klikając w niego kością");
		else if(player.getWorld() == Papi.Server.getSurvivalWorld())
			this.papiMenuPage.setItem(InventorySlot.ROW3_COLUMN5, Material.BONE, "&eOtrzymaj swojego wilka za darmo", SlotOption.GIVE_WOLF);
		else
			this.papiMenuPage.setItem(InventorySlot.ROW3_COLUMN5, Material.BONE, "&eOtrzymaj swojego wilka za darmo\n&7&oMożesz to zrobić tylko w świecie Survival");
		...
		else
		{
			...
			if(slotOption == SlotOption.GIVE_WOLF) {
				if(!WolfSecurityApi.existWolf(e.getPlayer()))
				{
					...
					if(result == -1)
						e.getPapiMenuPage().setItemNameAndLore(e.getSlot(), "&aPosiadasz swojego wilka!\n&7Możesz zarzącać wilkiem klikając w niego kością");
					if(result == 0)
						e.getPapiMenuPage().setItemNameAndLore(e.getSlot(), "&eOtrzymaj swojego wilka za darmo\n&7&oMożesz to zrobić tylko w świecie Survival");
					if(result == 1)
			...
			if(slotOption == SlotOption.ME_TP_TO_REGION) {
				...
				if(playerLocation.getWorld() != Papi.Server.getSurvivalWorld()) {
					e.getPapiMenuPage().setItemLoreLine(e.getSlot(), 2, "&c&oDostępne tylko na mapie Survival");
				}
				else if(WolfSecurityApi.existWolf(e.getPlayer()))
				{
					Wolf wolf = WolfSecurityApi.getWolfEntity(e.getPlayer());
					if(wolf.isValid())
					{
						e.getPlayer().closeInventory();
						wolf.teleport(playerLocation);
					}
					else
						e.getPapiMenuPage().setItemLoreLine(e.getSlot(), 2, "&cNie możemy odnaleźć twojego wilka :(");
				}
