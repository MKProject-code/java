	...
	private void setTargetTask(int taskid, Player player, int regionid, boolean wolfLastIsSitting) {
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(Plugin.plugin, new Runnable() {
			...
			public void run() {
				...
				if(player.isOnline() && Model.Player(player).getInsideRegionsId().contains((Object)regionid) && !Model.Region(regionid).isFamily(player) && !Model.Region(regionid).isFriend(player))
				{
