		...
		Bukkit.getScheduler().runTaskAsynchronously(Plugin.plugin, new Runnable() {
			...
			public void run() {
				for (Map.Entry<Integer, Integer> entry : InvSlot.users.entrySet()) {
	    			inv.setItem(entry.getKey(),WolfMenu.createGuiItemHeadParseLang(entry.getValue(), Message.getColorMessageLang("inventory.wolfmenu.wolf.manageenemies.submenu.showusers.submenu.head.slot_title", ImmutableMap.of(
