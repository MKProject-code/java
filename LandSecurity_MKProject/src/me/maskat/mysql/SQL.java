...
public class SQL
{
    ...
    public static int insertData_getId(final String columns, final String values, final String table) {
    	return MySQL.update("INSERT INTO " + table + " (" + columns + ") VALUES (" + values + ");", true);
    }
    ...
    public static ResultSet getResultSetWhere(final int limit, final String select, final String column, final String logic_gate, String data, final String table) {
        ...
        try {
        	final ResultSet rs = MySQL.query("SELECT "+select+" FROM " + table + " WHERE " + column + logic_gate + data + (limit > 0  ? " LIMIT "+String.valueOf(limit) : null) + ";");
            if (rs.next()) {
            	rs.beforeFirst();
                return rs;
            }
    ...
    public static Object get(final String selected, final String column, final String logic_gate, String data, final String table) {
        ...
        try {
            final ResultSet rs = MySQL.query("SELECT "+selected+" FROM " + table + " WHERE " + column + logic_gate + data + ";");
            if (rs.next()) {
                return rs.getObject(selected);
            }
        }
