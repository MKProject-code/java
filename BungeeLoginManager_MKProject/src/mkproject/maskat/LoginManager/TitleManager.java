	...
	public static void onPlayerJoin(PlayerJoinEvent e) {
		Bukkit.getScheduler().runTaskLater(Plugin.getPlugin(), () -> {
            ...
            if (Model.getPlayer(e.getPlayer()).isRegistered()) {
            	Plugin.getUnloggedAnimation().play(e.getPlayer(), p -> Model.getPlayer(e.getPlayer()).isLogged());
            } else {
            	Plugin.getUnregisteredAnimation().play(e.getPlayer(), p -> Model.getPlayer(e.getPlayer()).isLogged());
            }
        }, 20L);
...
//			if(!p.equals(player) && Model.getPlayer(p).isLogged())
//			{
