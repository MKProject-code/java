...
public class Function {
	...
	protected static void blockBreakCheckAround(Player player, Block block) {
		...
		if(Plugin.hiddenMaterials.contains(checkBlock.getType())) {
	...
	protected static void blocksAroundCheck(Player player, Block block, Block lookingBlockNear) {
		...
		if(Model.getPlayer(player).existHiddenBlock(block.getLocation()))
			return;
		...
		if(!showBlock) {
			if(Plugin.world.getBlockAt(x+1, y, z).getType().isTransparent())
				hideBlock = false;
			else if(Plugin.world.getBlockAt(x-1, y, z).getType().isTransparent())
				hideBlock = false;
			else if(Plugin.world.getBlockAt(x, y+1, z).getType().isTransparent())
				hideBlock = false;
			else if(Plugin.world.getBlockAt(x, y-1, z).getType().isTransparent())
				hideBlock = false;
			else if(Plugin.world.getBlockAt(x, y, z+1).getType().isTransparent())
				hideBlock = false;
			else if(Plugin.world.getBlockAt(x, y, z-1).getType().isTransparent())
				hideBlock = false;
		}
...
//	}
	...
	private static Material getRandomMaterialToShow() {
		return Plugin.materialsToShow.get(Papi.Function.randomInteger(0, Plugin.materialsToShow.size()-1));
	}
