...
//		private static final String ALLOW_ONLY_NUMBERS = "&cDozwolone tylko cyfry!";
	...
	private static final String NEED_CHOOSE_WORLD = "&cMusisz wybrać świat!";
	private static final String PLAYER_TELEPORT_ERROR = "&cTeleportacja nieudana";
	private static final String COMMAND_NO_USAGE_FOR_PLAYER = "&cNie możesz użyć tej komendy z poziomu gracza";
	private static final String COMMAND_NO_USAGE_FOR_CONSOLE = "&cNie możesz użyć tej komendy z poziomu konsoli";
//	private static final String COMMAND_NO_USAGE_FOR_COMMANDBLOCK = "&cNie możesz użyć tej komendy z poziomu CommandBlock'a";
...
//	}
	...
	private Map<Integer, String> defaultUsageList;
	...
	public CommandManager(String PERMISSION_PREFIX, CommandSender sender, Command command, String label, String[] args) {
		this.CommandManager_Constructor(PERMISSION_PREFIX, sender, command, label, args, null);
	}
	@Deprecated
	public CommandManager(String PERMISSION_PREFIX, CommandSender sender, Command command, String label, String[] args, List<String> defaultUsageList) {
		this.CommandManager_Constructor(PERMISSION_PREFIX, sender, command, label, args, defaultUsageList);
	}
	@Deprecated
	private void CommandManager_Constructor(String PERMISSION_PREFIX, CommandSender sender, Command command, String label, String[] args, List<String> defaultUsageList) {
		...
		this.defaultUsageList = new HashMap<>();
		if(defaultUsageList != null)
		{
	...
	//v2.0.0
	public CommandManager(JavaPlugin pluginExecutor, CommandSender sender, Command command, String label, String[] args) {
		this.CommandManager_Constructor(pluginExecutor, sender, command, label, args, null);
	}
	public CommandManager(JavaPlugin pluginExecutor, CommandSender sender, Command command, String label, String[] args, List<String> defaultUsageList) {
		this.CommandManager_Constructor(pluginExecutor, sender, command, label, args, defaultUsageList);
	}
	private void CommandManager_Constructor(JavaPlugin pluginExecutor, CommandSender sender, Command command, String label, String[] args, List<String> defaultUsageList) {
		this.pluginExecutor = pluginExecutor;
		
		this.PERMISSION_PREFIX = "mkp."+pluginExecutor.getName().toLowerCase()+".";
		this.lastCheckedPermission = this.PERMISSION_PREFIX+command.getName();
...
//	}
	...
	public boolean hasArgTabComplete(int intArg) {
		if(args.length == intArg)
			return true;
		return false;
	}
	
	public boolean hasArgTabComplete(int intArg, String argument) {
		if(args.length >= intArg+1 && args[intArg-1].equalsIgnoreCase(argument))
			return true;
		return false;
	}
	
	public boolean hasArgTabComplete(int intArg, String... arguments) {
		if(args.length >= intArg+1)
		{
	...
	public boolean setArgTabComplete(int intArg, List<String> arguments) {
		if(args.length == intArg)
		{
			completeTab = this.getListWhenContainsFirstString(argsDefault[intArg-1], arguments);
			return true;
		}
		return false;
	}
	
	public boolean hasArgOrSetTabComplete(int intArg, List<String> arguments) {
		if(args.length == intArg)
		{
			completeTab = this.getListWhenContainsFirstString(argsDefault[intArg-1], arguments);
		}
		else if(args.length > intArg)
		{
			for(String argument : arguments)
			{
				if(args[intArg-1].equalsIgnoreCase(argument))
	...
	public boolean doReturn() {
		...
		else if(this.console != null)//TODO: if(this.isConsole() || this.isCommandBlock())
		{
			if(this.pluginExecutor != null)
				Message.sendConsoleInfo(this.pluginExecutor, returnMessage);
			else
				Message.sendConsoleInfo(Plugin.getPlugin(), "[CommandManager] Command '"+this.command+"' generated return message: "+returnMessage);
		}
...
//	}
	...
	public boolean isPlayer() {
		...
		setReturnMessage(COMMAND_NO_USAGE_FOR_PLAYER);
		return false;
	}
	...
	public boolean isConsole() {
		...
		setReturnMessage(COMMAND_NO_USAGE_FOR_CONSOLE);
		return false;
	}
...
//			return true;
//		
//		setReturnMessage(COMMAND_NO_USAGE_FOR_COMMANDBLOCK);
//		return false;
//	}
	...
	private Player getChosenPlayer(Player otherPlayer, boolean canChoseSelf, boolean checkImmunitet) {
		...
		else if(this.player != null && !player.hasPermission(lastCheckedPermission+".otherplayer"))
		{
			setReturnMessage(getDenyMessage(lastCheckedPermission+".otherplayer"));
			return null;
		}
		else if(this.player != null && checkImmunitet && otherPlayer.hasPermission(lastCheckedPermission+".immunitet") && !player.hasPermission(lastCheckedPermission+".bypass.immunitet"))
		{
	...
	public World getChosenWorldFromArg(int argInt, boolean canChoseSelfWorld) {
		if(this.getArg(argInt) == null) {
			if(this.player == null)
			{
				setReturnMessage(NEED_CHOOSE_WORLD);
				return null;
			}
			
			if(canChoseSelfWorld)
			{
				if(!player.hasPermission(lastCheckedPermission+".accessworld."+player.getWorld().getName()))
				{
					setReturnMessage(getDenyMessage(lastCheckedPermission+".accessworld."+player.getWorld().getName()));
					return null;
				}
		...
		if(otherWorld != null)
		{
			...
			{
				if(canChoseSelfWorld && player.hasPermission(lastCheckedPermission+".accessworld."+otherWorld.getName()))
					return otherWorld;
			...
			if(!player.hasPermission(lastCheckedPermission+".otherworld"))
			{
				setReturnMessage(getDenyMessage(lastCheckedPermission+".otherworld"));
				return null;
			}
			
			if(!player.hasPermission(lastCheckedPermission+".accessworld."+otherWorld.getName()))
			{
				setReturnMessage(getDenyMessage(lastCheckedPermission+".accessworld."+otherWorld.getName()));
				return null;
			}
	...
	public World getChosenWorldFromArgWithThis(int argInt, boolean canChoseSelfWorld) {
		if(this.getArg(argInt) == null || this.getArg(argInt).equalsIgnoreCase("this")) {
			if(this.player == null)
			{
				setReturnMessage(NEED_CHOOSE_WORLD);
				return null;
	...
	public void sendMessage(String message) {
		...
		{
			if(this.pluginExecutor != null)
				Message.sendConsoleInfo(this.pluginExecutor, message);
			else
				Message.sendConsoleInfo(Plugin.getPlugin(), "[CommandManager] Command '"+this.command+"' generated message: "+message);
		}
	}
	...
	public List<String> getWorldsNameListWithThis() {
		List<String> worldNameList = new ArrayList<>();
		worldNameList.add("this");
		for(World world : Bukkit.getWorlds()) {
			worldNameList.add(world.getName());
		}
		return worldNameList;
	...
	public List<String> getWorldsNameListWithAccess(boolean canChoseSelfWorld) {
		List<String> worldNameList = new ArrayList<>();
		for(World world : Bukkit.getWorlds()) {
			if(world.equals(player.getWorld()))
			{
				if(canChoseSelfWorld && player.hasPermission(lastCheckedPermission+".accessworld."+world.getName()))
					worldNameList.add(world.getName());
			}
			else if(player.hasPermission(lastCheckedPermission+".otherworld"))
			{
				if(player.hasPermission(lastCheckedPermission+".accessworld."+world.getName()))
					worldNameList.add(world.getName());
			}
		}
		return worldNameList;
