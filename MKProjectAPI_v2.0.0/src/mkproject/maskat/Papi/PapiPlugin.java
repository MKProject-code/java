...
import mkproject.maskat.Papi.Bungee.MessageListener;
...
public class PapiPlugin extends JavaPlugin {
	...
	public void onEnable() {
		...
		if(this.getConfig().getBoolean("Plugin.UseVault")) {
	        if (!setupEconomy() ) {
	        	getLogger().warning("*********************************************************************");
	        	getLogger().warning("********* ERROR Economy: Disabled due to no Vault dependency found! *********");
	        	getLogger().warning("*********************************************************************");
	            getServer().getPluginManager().disablePlugin(this);
	            return;
	        }
	        if (!setupPermissions() ) {
	        	getLogger().warning("*********************************************************************");
	        	getLogger().warning("********* ERROR Permission: Disabled due to no Vault dependency found! *********");
	        	getLogger().warning("*********************************************************************");
	            getServer().getPluginManager().disablePlugin(this);
	            return;
	        }
	        if (!setupChat() ) {
	        	getLogger().warning("*********************************************************************");
	        	getLogger().warning("********* ERROR Chat: Disabled due to no Vault dependency found! *********");
	        	getLogger().warning("*********************************************************************");
	            getServer().getPluginManager().disablePlugin(this);
	            return;
	        }
        ...
        PapiServer.initNetherWorld(Bukkit.getServer().getWorld("world_nether"));
        PapiServer.initTheEndWorld(Bukkit.getServer().getWorld("world_the_end"));
        ...
        PapiBungee.checkIfBungeeEnabled();
        
        this.getServer().getMessenger().registerOutgoingPluginChannel(this, "BungeeCord");
        this.getServer().getMessenger().registerIncomingPluginChannel(this, "BungeeCord", new MessageListener());
        
        PapiBungee.registerIncomingPluginChannel(this, BungeeChannel.GlobalChat, new MessageListener());
